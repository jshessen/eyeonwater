...
try:
    from pyonwater import enforce_monotonic_total, filter_points_after
except ImportError:
    # Inline fallback for pyonwater < 0.4.0.
    # Removed in feat/unified-sensor once pyonwater>=0.4.0 is on PyPI.
        ...
        *,
        ...
        for point in points:
            ...
            if last_value is None:
                ...
                last_value = reading
            normalized.append(
                DataPoint(dt=point.dt, reading=reading, unit=point.unit),
            )
        return normalized
